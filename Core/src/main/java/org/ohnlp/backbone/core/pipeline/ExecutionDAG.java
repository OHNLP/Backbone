...
public class ExecutionDAG {
    ...
    public void planDAG(Pipeline pipeline) {
        ...
        Map<String, PCollection<Row>> outputCollsByID = new HashMap<>();
        ...
        for (String extractID : extracts) {
            ...
            Schema out = extract.calculateOutputSchema(null);
            PCollection<Row> df = start.apply("Step-" + extractID, extract).setRowSchema(out);
    ...
    private Set<PipelineBuilder.InitializedPipelineComponent> runNextBatchOrDefer(Pipeline p, Set<PipelineBuilder.InitializedPipelineComponent> componentsToProcess, Map<String, PCollection<Row>> outputCollsByID) {
        ...
        for (PipelineBuilder.InitializedPipelineComponent component : componentsToProcess) {
            ...
            if (component.getComponent() instanceof Transform) {
                PCollection<Row> df = outputCollsByID.get(component.getInputs().get(0)).apply("Step-" + component.getComponentID(), (Transform)component.getComponent());
    ...
    private void checkPipelineValidity() {
        ...
        componentsByID.forEach((id, cmp) -> {
            ...
            if (cmp.getInputs() != null && cmp.getInputs().size() != 1 && cmp.getComponent() instanceof Transform) {
